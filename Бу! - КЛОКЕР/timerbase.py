...
class StealthTimer:
    ...
    def check_offline_time(self):
            """Проверяет, сколько компьютер был выключен"""
            heartbeat_file = os.path.join(BASE_DIR, "last_seen.txt")
            if os.path.exists(heartbeat_file):
                try:
                    with open(heartbeat_file, "r") as f:
                        last_time_str = f.read().strip()
                    last_time = datetime.strptime(last_time_str, '%Y-%m-%d %H:%M:%S')
                    offline_duration = datetime.now() - last_time
                    
                    # Если прошло больше 1 минуты (чтобы не спамить при перезагрузке софта)
                    if offline_duration > timedelta(minutes=1):
                        off_str = str(offline_duration).split('.')[0]
                        messagebox.showinfo("ПК был отключен", f"Компьютер не работал: {off_str}")
                except:
                    pass

    def run_heartbeat(self):
            """Фоновый процесс: раз в 10 секунд пишет текущее время в файл"""
            heartbeat_file = os.path.join(BASE_DIR, "last_seen.txt")
            while True:
                # Раз в неделю (по понедельникам) можно было бы чистить логи, 
                # но так как мы ПЕРЕЗАПИСЫВАЕМ одну строку, файл всегда весит 1 КБ.
